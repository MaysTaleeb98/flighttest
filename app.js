...
/*
list the reservations on the system using this route: http://localhost:5000/reservations
*/
...
app.get('/invoice', (req,res) => {
    /*
    Reservations.Fare.aggregatedb.orders.aggregate([
        {
           $lookup: {
              from: " Reservations",
    ...
    Reservations.find({Customer_name: {$regex: searchedFiled}},  {Flight_number:1,Date:1}).then(result => {
        res.status('200').json({
            status : 200,
            number_of_results: result.length,
            Flights :result
